...
of $E_f$ has large coefficients.  For example, the elliptic curve given by For example,
the curve
$$y^2 = x^3 + \left(186921 a - 301671\right)x + \left(47055384 a -74637018\right)$$ 
has conductor $\n = -38 \vphi + 10$, but  naive enumeration would take
 about $10^{15}$ {\em years} to  search for all elliptic curves
 with coefficients $a,b$ of the form  $w+z\vphi$ with $|w|,|z| <10^8$, 
at the speed of a billion curves per second.
...
B := 1831$, we have $C=31$ and square-free $d_1$ 
such that $|\bN(d_1)| \leq \sqrt{B/C} \sim 7.6\ldots$. 
Thus $d_1 \in \{1, 2,\vphi, 2\vphi\}$ and checking all possibilities for
$\vphi^{\delta} d_0 d_1$ we found the curve $E^{-\vphi-2}$ having
...
conductor $126976=2^{12}\cdot 31$.o find all twists $E^d$ with norm conductor
 at most $B$, we twist $E$ by all $d$ of the form $\pm \vphi^{\delta} d_0 d_1$,
 where $\delta\in \{0,1\}$, $d_0$ is a product of a fixed choice of generators for
the prime divisors of $\n$, $d_1$ is a squarefree product of a
 fixed choice of generators of primes not dividing $\n$, and
 $|\bN(d_1)| \leq \sqrt{B/C}$, where $C$ is the norm of the product of
the primes that exactly divide $\n$. We know from \ref{twist:bound} that 
this search is exhaustive.
...
B := 1831$, we have $C=31$ and square-free $d_1$ such that
 $|\bN(d_1)| \leq \sqrt{B/C} \sim 7.6\ldots$. Thus $d_1 \in \{1, 2, \vphi, 2\vphi\}$ 
and checking all possibilities for $\vphi^{\delta} d_0 d_1$, we found
 the curve $E^{-\vphi-2}$ having norm conductor $775$ and the
 curve $E^{5\vphi-3}$ having norm conductor $961$.
 Other twists have larger norm conductors, e.g., $E^2$ has norm
...
points on $E$ defined over $\cO_K$ and was in general not as successful.
Using Cremona's $\textsc{Magma}$ implementation of the algorithm, we found 
a few dozen curves, eg.,  
...
Given a curve $E'$ and $f$ corresponding to an isogeny class represented 
by $E$ such that  $E[\ell] \ncisom E'[\ell]$, where $\ell$ is prime and
 $E[\ell]$ is an irreducible representation of $\text{Gal}(\overline{F}/F)$.
  If $\ell=5,7$ then we can use techniques due to Fisher and Stoll (NEEDS REFERENCE!!) to
 search through the finite list of curves having their associated mod~$\ell$
 representation isomorphic to $E[\ell]$.

(ISSUES! -- for instance, what is E'? Why are the reps isomorphic just based 
off the Hecke values) We used this approach to find the curve $E$ given by
$$
y^2 + \vphi{}xy = x^3 + \left(\vphi{} - 1\right)x^2 + \left(-257364 \vphi{} - 159063\right)x + \left(-75257037 \vphi{} - 46511406\right)
$$
having norm conductor 1476. From the values of $a_\p$, (WHY IS THIS TRUE?!!)
$E[7] \ncisom E'[7]$, where $E'$ has norm conductor 369. Fisher
used a program in \textsc{Magma} to find rational points on a certain
quartic surface that parametrizes curves with the same $E'[7]$ and found
$E$ by this method.
...
outlines some methods to find an elliptic curve $E$ corresponding to
 $f$, a Hilbert modular form of level $\n$ over any real quadratic field. 
Formally, these methods are not proven to be better than a direct search, as they
involve making a large and a priori undefined number of {\em guesses}. 
 The method requires the computation of many Hecke eigenvalues
of $f$ which takes a lot of time. However, it works extremely
 well in many cases, and after tuning it using large tables of
 curves that we had already computed, we were able
to use it to find curves that we would have had no hope of
finding otherwise; we give an example of one of these curves below.
 This is the {\em first} general implementation of the algorithm from
\cite{dembele:elliptic-curves-quadratic-fields} for $F = \Q(\sqrt{5})$.

When the level $\n$ is not a square, \dembele's method relies on computing
...
required are $\n:=(N)$ and the L-series $L(s,f)$ (NORMALIZED HOW?) where $N$ is 
totally positive. We also assume that we have sufficiently many Hecke 
eigenvalues of $f$.
...
possibilities for $j(E)$ as an algebraic number. We construct a curve $E'$
corresponding to each possibility for $j(E)$. By computing a few $\ap{}(E)$, we
should be able to determine whether we have chosen the correct $j$-invariant in which case
 $E'$ will be a twist of $E$. We can then recognize which twist it is
in order to recover $E$.

In practice, of course, as we have limited precision, and as $j(E)$ will not be
an algebraic integer it may not be feasible to directly determine its value,
especially if its denominator is large. %Still, the above approach may be useful. UNNECESSARY?
